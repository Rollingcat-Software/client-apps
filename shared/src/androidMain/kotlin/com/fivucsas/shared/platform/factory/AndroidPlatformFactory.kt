...
/**
 * Android Platform Service Factory
 *
 * Creates Android-specific implementations of platform services.
 * The main DI is handled by Koin in PlatformModule.android.kt;
...
class AndroidPlatformServiceFactory : PlatformServiceFactory {
    override fun createCameraService(): ICameraService {
        // Actual camera service requires Context, so return stub.
        // Real camera service is provided via Koin DI.
        return StubCameraService()
    }

    override fun createLogger(): ILogger = AndroidLogger()

    override fun createSecureStorage(): ISecureStorage {
        // Actual secure storage requires Context, provided via Koin DI.
        return StubSecureStorage()
    }
...
private class AndroidLogger : ILogger {
    ...
    override fun error(tag: String, message: String, throwable: Throwable?) { println("E/$tag: $message") }
    override fun verbose(tag: String, message: String) { println("V/$tag: $message") }
}
